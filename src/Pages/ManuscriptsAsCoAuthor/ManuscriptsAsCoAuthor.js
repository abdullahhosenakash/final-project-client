...
const ManuscriptsAsCoAuthor = () => {
  ...
  useEffect(() => {
    ...
    const authorLink = `https://final-project-server-liard.vercel.app/authorManuscripts?authorEmail=${user?.email}`;
    const editorLink =
      'https://final-project-server-liard.vercel.app/manuscriptsAsCoAuthor';
    if (!userRole) return;
    fetch(
      userRole === 'author'
        ? authorLink
